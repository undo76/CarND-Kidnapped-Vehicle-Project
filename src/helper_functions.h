...
inline double multivariate_gaussian_pdf(const double mu_x, const double mu_y,
                                        const double sig_x, const double sig_y,
                                        const double x, const double y) {
  auto gauss_norm = (1. / (2. * M_PI * sig_x * sig_y));
  auto exponent = ((x - mu_x) * (x - mu_x) / (2 * sig_x * sig_x)) +
